...
| `haze-ui` | Component library (Card, Input, Textarea, TagInput, Chip, ...), also re-exports `useControl`; since 1.11 the form layer (`FormItem` / `useFormControl`, react-f0rm ↔ react-use-control bridge) is in the main barrel. CSS is imported per-component from `src/styles.ts` (`haze-ui/css/tokens.css` + each used component's `haze-ui/css/<name>.css`; family files absorb sub-components: list ← ListItem, tag-group ← TagGroupItem, navigation-bar ← NavLink, typography ← Title/Text, toast ← ToastContainer, form ← FormItem) |
