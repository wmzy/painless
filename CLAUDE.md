...
**Editor unsaved-changes guard (three channels):** `useBlocker` (react 1.6, on core's `setBlocker` veto) covers every leaving channel: ① in-app navigation — a synchronous predicate; `isDirty(form)` pops a `ConfirmDialog` and vetoes (`confirmRef` stashes the destination; confirming resets to clean then `navigate`s to it); ② browser back/forward (POP) — same predicate, and a vetoed POP is auto-compensated by the library with a reverse `go()`, so the URL never moves; ③ full unload (refresh/close) — `beforeunload` with the legacy `returnValue` double-write (some browsers ignore `preventDefault` alone). After a successful save, `setInitialValues(form, values)` runs *before* `navigate` — otherwise live values still differ from the old initialValues and the post-save navigation itself would be vetoed.
