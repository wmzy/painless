...
// fixtures/mockApi/login 等共享定义已抽至 ./helpers（prod/visual spec
// 复用），此处只 import；用例体零改动。
import {
  article1,
  article2,
  author,
...
// ---------------------------------------------------------------------------
// a11y：@axe-core/playwright 对 mock 数据渲染出的主要页面做 WCAG
// 2.0/2.1/2.2 A/AA 系统化扫描（复用上面的 mockApi/login fixtures，
// 网络层全 mock，未预期端点 404）。违例处理约定：painless 视图侧问题
//（缺 label、aria 链路断裂等）修视图代码；haze-ui 组件自身的问题
//（如 token 对比度）不改库——在扫描处 disable 对应规则并注明
//「haze-ui 侧问题，待库侧审核」。
// ---------------------------------------------------------------------------

// WCAG 2.0/2.1/2.2 的 A/AA 规则集（axe-core 官方 tag 体系）。2.2 在
// 本仓锁定的 axe-core 4.13 上的实际覆盖（读 axe.js 源 + 实测确认）：
// wcag22aa 目前唯一携带者是 target-size（2.5.8 目标尺寸，规则默认
// enabled:false，但 runOnly 按 tag 选择不受该默认值阻拦——matchTags
// 只看 tag 匹配，实测 /login 页 10 个交互节点全部通过）；wcag22a 尚无
// 任何规则携带（axe 后续版本补 2.2 A 级自动规则时此 tag 自动生效，
// 前向保留）。升级 axe-core 后如 2.2 覆盖变宽，新违例按本段分流约定
// 处置。
const A11Y_TAGS = [
  'wcag2a',
  'wcag2aa',
